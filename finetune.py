...
def parse_command_line_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help='Without this option, when the program is running in distributed '
            ...
            'feature is disabled and the per-GPU batch size will be kept '
            'constant regardless of CUDA device numbers.'
    )
    parser.add_argument(
        '-q', '--no-progressbars',
        action='store_true',
    ...
    parser.add_argument(
        ...
        help='Run the training script in testing mode. Used for debugging. '
            ...
            'always be set to "tests/data/finetune_hyperparams.csv". If a '
            '"test" finetuning run directory already exists, this will be '
