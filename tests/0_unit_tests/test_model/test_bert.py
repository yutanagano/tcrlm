...
def bert():
    ...
    self_attention_stack = SelfAttentionStackWithBuiltins(num_layers=2, d_model=D_MODEL, nhead=2)
    mlm_token_prediction_projector = AminoAcidTokenProjector(d_model=D_MODEL)
    vector_representation_delegate = AveragePoolVectorRepresentationDelegate(self_attention_stack=self_attention_stack)
...
def mock_tokenised_tcrs():
    return torch.ones((BATCH_SIZE, TOKENISED_TCR_LENGTH, TOKEN_DIMENSIONALITY), dtype=torch.long)
