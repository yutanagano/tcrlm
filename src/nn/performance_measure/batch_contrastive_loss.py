...
class BatchContrastiveLoss(Module):
    ...
    *A caveat is that in order to prevent the exp terms from suffering from overflow, the dot products are all normalised.
    ...
    def forward(
        ...
        dot_products = (
            torch.matmul(tcr_representations, tcr_representations.T) / self._temp
        )
    ...
    def _ajust_dot_products_to_prevent_overflow(
        self, dot_products: FloatTensor
    ) -> FloatTensor:
    ...
    def _get_positives_term(
        self, dot_products: FloatTensor, positives_mask: BoolTensor
    ) -> FloatTensor:
        ALONG_COMPARISONS_DIM = 1
        contributions_from_positive_comparisons = torch.sum(
            dot_products * positives_mask, dim=ALONG_COMPARISONS_DIM
        )
