...
class TrainingManager:
    ...
    def _launch_training_processes(self) -> None:
        ...
        print(f"Launching {self._num_gpus_available} training processes on port {port}...")
        multiprocessing.spawn(self._training_process, args=(port,), nprocs=self._num_gpus_available)
    ...
    def _training_process(self, gpu_index: int, port: int) -> None:
        ...
        valid_metrics = self._validate_and_return_metrics_for(training_object_collection)
        ...
        for epoch in range(1, self._get_num_epochs() + 1):
            ...
            train_metrics = self._run_training_epoch_and_return_metrics_for(training_object_collection)
        ...
        if current_process_using_first_gpu:
            ...
            self._save_training_results(model=unwrapped_bert_model, metric_log=metric_log)
    ...
    def _run_training_epoch_and_return_metrics_for(self, training_object_collection: TrainingObjectCollection) -> dict:
        return self._training_delegate.run_training_epoch_and_return_metrics_for(training_object_collection)

    def _validate_and_return_metrics_for(self, training_object_collection: TrainingObjectCollection) -> dict:
        return self._training_delegate.validate_and_return_metrics_for(training_object_collection)
    ...
    def _make_model_save_dir_and_return_path(self) -> Path:
        ...
        try:
            ...
            model_save_dir = self._make_renamed_version_of_dir_and_return_path(model_save_dir)
    ...
    def _make_renamed_version_of_dir_and_return_path(self, path_to_original_dir: Path) -> Path:
    ...
    def _setup_ddp(self, gpu_index: int, port: int) -> None:
        ...
        distributed.init_process_group(backend="nccl", rank=gpu_index, world_size=self._num_gpus_available)
