...
class BetaCdrLevenshtein(TcrMetric):
    ...
    def calc_cdist_matrix(self, anchor_tcrs: DataFrame, comparison_tcrs: DataFrame) -> ndarray:
        ...
        comparison_trbvs = self._add_first_allele_designation_if_none_present(comparison_trbvs)

        anchor_vcdrs = self._get_vcdrs(anchor_trbvs)
        comparison_vcdrs = self._get_vcdrs(comparison_trbvs)

        cdist_cdr1 = process.cdist(anchor_vcdrs.CDR1, comparison_vcdrs.CDR1, scorer=Levenshtein.distance)
        cdist_cdr2 = process.cdist(anchor_vcdrs.CDR2, comparison_vcdrs.CDR2, scorer=Levenshtein.distance)
        cdist_cdr3 = process.cdist(anchor_tcrs.CDR3B, comparison_tcrs.CDR3B, scorer=Levenshtein.distance)
