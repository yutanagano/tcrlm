...
class BetaMlTcrDist(TcrMetric):
    ...
    def calc_cdist_matrix(
        ...
        packaged_anchor_tcrs = anchor_tcrs.apply(self._package_sequence, axis=1)
        packaged_comparison_tcrs = comparison_tcrs.apply(self._package_sequence, axis=1)
        ...
        for anchor_index, comparison_index in itertools.product(
            range(num_anchors), range(num_comparisons)
        ):
            ...
            mat = self._nmat_gen(
                [
                    [
                        ...
                        comparison_tcr[0],
                        (
                            self._V_TO_NUMBER[anchor_tcr[1]],
                            self._V_TO_NUMBER[comparison_tcr[1]],
                        ),
                    ]
                ],
                dr,
        ...
        for ind1, (s1, s2, Vg) in enumerate(pairs):
            ...
            for ind, (op, i1, i2) in enumerate(steps):
                if op == "replace":  # 1 is replace
                    ii1, ii2 = sorted(
                        [self._AA_TO_NUMBER[s1[i1]], self._AA_TO_NUMBER[s2[i2]]]
                    )
                    matrix1[ind1, (2 + 4 * ind) : (2 + 4 * (ind + 1))] = [
                        LOP,
                        i1,
                        ii1,
                        ii2,
