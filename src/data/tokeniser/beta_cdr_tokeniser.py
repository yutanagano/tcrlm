...
from src.data.tokeniser.token_indices import AminoAcidTokenIndex, BetaCdrCompartmentIndex
...
class BetaCdrTokeniser(Tokeniser):
    ...
    def tokenise(self, tcr: Tcr) -> Tensor:
        initial_cls_vector = (AminoAcidTokenIndex.CLS, 0, 0, BetaCdrCompartmentIndex.NULL)
        
        cdr1b = self._convert_to_numerical_form(tcr.cdr1b_sequence, BetaCdrCompartmentIndex.CDR1)
        cdr2b = self._convert_to_numerical_form(tcr.cdr2b_sequence, BetaCdrCompartmentIndex.CDR2)
        cdr3b = self._convert_to_numerical_form(tcr.junction_b_sequence, BetaCdrCompartmentIndex.CDR3)
    ...
    def _convert_to_numerical_form(self, aa_sequence: Optional[str], cdr_index: BetaCdrCompartmentIndex) -> List[Tuple[int]]:
