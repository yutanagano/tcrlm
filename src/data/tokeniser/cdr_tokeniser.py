...
class CdrTokeniser(Tokeniser):
    ...
    def tokenise(self, tcr: Tcr) -> Tensor:
        ...
        cdr1a = self._convert_to_numerical_form(tcr.cdr1a_sequence, CdrCompartmentIndex.CDR1A)
        cdr2a = self._convert_to_numerical_form(tcr.cdr2a_sequence, CdrCompartmentIndex.CDR2A)
        cdr3a = self._convert_to_numerical_form(tcr.junction_a_sequence, CdrCompartmentIndex.CDR3A)

        cdr1b = self._convert_to_numerical_form(tcr.cdr1b_sequence, CdrCompartmentIndex.CDR1B)
        cdr2b = self._convert_to_numerical_form(tcr.cdr2b_sequence, CdrCompartmentIndex.CDR2B)
        cdr3b = self._convert_to_numerical_form(tcr.junction_b_sequence, CdrCompartmentIndex.CDR3B)

        all_cdrs_tokenised = [initial_cls_vector] + cdr1a + cdr2a + cdr3a + cdr1b + cdr2b + cdr3b
    ...
    def _convert_to_numerical_form(self, aa_sequence: Optional[str], cdr_index: CdrCompartmentIndex) -> List[Tuple[int]]:
