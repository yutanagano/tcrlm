...
class AucByLevenshteinGroups(Analysis):
    ...
    def run(self) -> AnalysisResult:
        ...
        return AnalysisResult("auc_by_levenshtein_groups", results=self._results, figures=self._figures)
    ...
    def _get_cospecificity_labels(self, dataset: DataFrame) -> ndarray:
        ...
        cospecificity_table = epitopes_as_ndarray[:,np.newaxis] == epitopes_as_ndarray[np.newaxis,:]
    ...
    def _analyse_model_performance(self, dataset_name: str, dist_table: DataFrame) -> None:
        ...
        for dist in range(1, max_levenshtein_distance+1):
            if not self._enough_examples_at_levenshtein_distance(dist_table, dist):
                continue
            performance_dict[dist] = self._analyse_model_performance_at_given_dist(dist_table, dist)

        self._results[dataset_name] = performance_dict
        self._figures[f"roc_auc_{dataset_name}"] = self._generate_roc_auc_plot(dataset_name, performance_dict)

    def _enough_examples_at_levenshtein_distance(self, dist_table: DataFrame, dist: int) -> bool:
        filtered_dist_table = dist_table[dist_table.levenshtein == dist]
        if filtered_dist_table.empty:
            return False

        cospecificity_group_sizes = filtered_dist_table.groupby("cospecificity_label").size()
    ...
    def _prepare_balanced_dist_table_for_distance(self, dist_table: DataFrame, dist: int) -> DataFrame:
        filtered_dist_table = dist_table[dist_table.levenshtein == dist]
        number_of_pairs_to_sample_from_each_group = filtered_dist_table.groupby("cospecificity_label").size().min()

        positive_pairs = filtered_dist_table[filtered_dist_table.cospecificity_label == True]
        negative_pairs = filtered_dist_table[filtered_dist_table.cospecificity_label == False]

        positive_pairs_subsampled = positive_pairs.sample(n=number_of_pairs_to_sample_from_each_group)
        negative_pairs_subsampled = negative_pairs.sample(n=number_of_pairs_to_sample_from_each_group)

        return pd.concat([positive_pairs_subsampled, negative_pairs_subsampled]).reset_index(drop=True)
