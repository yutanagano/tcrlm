...
class MetricCalibrationAnalysis(Analysis):
    def run(self) -> AnalysisResult:
        ...
        for dataset_name, dataset in self._labelled_data.items():
            ep_matched_dist_histogram = (
                self._compute_epitope_matched_distance_histogram_from_dataset(dataset)
            )
            results_dict[
                f"ep_matched_dist_hist_{dataset_name}"
            ] = ep_matched_dist_histogram
    ...
    def _sample_n_bg_distances_and_save_to_cache(self, n: int) -> None:
        ...
        with self._model_computation_cacher.get_appendable_buffer(
            ...
            for _ in tqdm(range(n)):
                anchor_index, comparison_index = random.sample(
                    range(len(bg_data_with_repeated_clones)), k=2
                )
                relevant_tcrs = bg_data_with_repeated_clones.iloc[
                    [anchor_index, comparison_index]
                ]
                distance = self._model.calc_pdist_vector(relevant_tcrs).item()
                f.write(f"{distance}\n")

    def _compute_epitope_matched_distance_histogram_from_dataset(
    ...
    def _generate_histogram_from_sample(self, sample_of_distances: ndarray) -> ndarray:
        ...
        histogram, _ = np.histogram(
            sample_of_distances, bins=distance_bins, density=True
        )
        return histogram
