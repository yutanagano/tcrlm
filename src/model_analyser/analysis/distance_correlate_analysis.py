...
class DistanceCorrelateAnalysis(Analysis):
    ...
    def _get_all_possible_edits_and_resulting_distances(
        ...
        distances_between_original_and_edited_tcrs = self._model.calc_cdist_matrix(
            original_tcr, edited_tcrs
        ).squeeze()
        return zip(edits, distances_between_original_and_edited_tcrs)

    def _get_edits_and_resulting_variants_of_tcr(
        self, tcr: DataFrame
    ) -> Tuple[Iterable[TcrEdit], DataFrame]:
        trav_edits, tcrs_with_trav_edits = self._get_all_trav_edits_and_resulting_tcrs(
            tcr
        )
        trbv_edits, tcrs_with_trbv_edits = self._get_all_trbv_edits_and_resulting_tcrs(
            tcr
        )
        (
            alpha_junction_edits,
            tcrs_with_alpha_junction_edits,
        ) = self._get_all_alpha_junction_edits_and_resulting_tcrs(tcr)
        (
            beta_junction_edits,
            tcrs_with_beta_junction_edits,
        ) = self._get_all_beta_junction_edits_and_resulting_tcrs(tcr)
        ...
        all_edited_tcrs = [
            *tcrs_with_trav_edits,
            *tcrs_with_trbv_edits,
            *tcrs_with_alpha_junction_edits,
            *tcrs_with_beta_junction_edits,
        ]
    ...
    def _get_all_trav_edits_and_resulting_tcrs(
        self, tcr: DataFrame
    ) -> Tuple[Iterable[TcrEdit], Iterable[DataFrame]]:
        # TODO
        return ([], [])

    def _get_all_trbv_edits_and_resulting_tcrs(
        self, tcr: DataFrame
    ) -> Tuple[Iterable[TcrEdit], Iterable[DataFrame]]:
        # TODO
        return ([], [])

    def _get_all_alpha_junction_edits_and_resulting_tcrs(
        self, tcr: DataFrame
    ) -> Tuple[Iterable[TcrEdit], Iterable[DataFrame]]:
        # TODO
        return ([], [])

    def _get_all_beta_junction_edits_and_resulting_tcrs(
        self, tcr: DataFrame
    ) -> Tuple[Iterable[TcrEdit], Iterable[DataFrame]]:
        edits = []
        edited_tcrs = []
        ...
        for edit_index in range(max_junction_length_post_edit):
            ...
            ) = self._get_aa_insertions_at_index_and_resulting_junctions(
                junction, edit_index
            )
            tcrs_with_insertion = [
                self._get_tcr_with_edited_component(
                    tcr, "CDR3B", junction_with_insertion
                )
            ...
            if edit_index < len(junction):
                (
                    deletions,
                    junctions_with_deletion,
                ) = self._get_aa_deletions_at_index_and_resulting_junctions(
                    junction, edit_index
                )
                tcrs_with_deletion = [
                    self._get_tcr_with_edited_component(
                        tcr, "CDR3B", junction_with_deletion
                    )
                    for junction_with_deletion in junctions_with_deletion
                ]

                (
                    substitutions,
                    junctions_with_substitution,
                ) = self._get_aa_substitutions_at_index_and_resulting_junctions(
                    junction, edit_index
                )
                tcrs_with_substitution = [
                    self._get_tcr_with_edited_component(
                        tcr, "CDR3B", junction_with_substitution
                    )
    ...
    def _get_aa_insertions_at_index_and_resulting_junctions(
        self, junction: str, insertion_index: int
    ) -> Tuple[Iterable[TcrEdit], Iterable[str]]:
        insertions = []
        junctions_with_insertion = []

        insertion_position = self._get_edit_position_for_junction_from_edit_index(
    ...
    def _get_aa_deletions_at_index_and_resulting_junctions(
        self, junction: str, deletion_index: int
    ) -> Tuple[Iterable[TcrEdit], Iterable[str]]:
        deletion_position = self._get_edit_position_for_junction_from_edit_index(
            junction, deletion_index
        )
    ...
    def _get_aa_substitutions_at_index_and_resulting_junctions(
        self, junction: str, substitution_index: int
    ) -> Tuple[Iterable[TcrEdit], Iterable[str]]:
        substitutions = []
        junctions_with_substitution = []

        substitution_position = self._get_edit_position_for_junction_from_edit_index(
    ...
    def _get_edit_position_for_junction_from_edit_index(
        self, junction: str, edit_index: int
    ) -> Position:
    ...
    def _get_tcr_with_edited_component(
        self, tcr: DataFrame, component_name: str, new_component_value
    ) -> DataFrame:
