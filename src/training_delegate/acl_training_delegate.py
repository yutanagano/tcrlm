...
class AclTrainingDelegate(TrainingDelegate):
    def run_training_epoch_and_return_metrics_for(self, training_object_collection: TrainingObjectCollection) -> dict:
        ...
        cross_entropy_loss_fn = training_object_collection.loss_functions["cross_entropy_loss"]
        contrastive_loss_fn = training_object_collection.loss_functions["contrastive_loss"]
        ...
        for anchor_tcrs, positive_pair_tcrs, masked_tcrs, mlm_targets in tqdm(dataloader, disable=current_process_not_on_first_gpu):
            ...
            anchor_tcr_embeddings, positive_pair_tcr_embeddings, mlm_logits = model(anchor_tcrs, positive_pair_tcrs, masked_tcrs)

            optimiser.zero_grad()
            loss =\
                contrastive_loss_fn(anchor_tcr_embeddings, positive_pair_tcr_embeddings) + \
    ...
    def validate_and_return_metrics_for(self, training_object_collection: TrainingObjectCollection) -> dict:
        ...
        for anchor_tcrs, positive_pair_tcrs, masked_tcrs, mlm_targets in tqdm(dataloader, disable=current_process_not_on_first_gpu):
            ...
            anchor_tcr_embeddings = model.module.bert.get_vector_representations_of(anchor_tcrs)
            positive_pair_tcr_embeddings = model.module.bert.get_vector_representations_of(positive_pair_tcrs)

            model.eval()
            mlm_logits = model.module.bert.get_mlm_token_predictions_for(masked_tcrs)

            contrastive_loss = contrastive_loss_fn(anchor_tcr_embeddings, positive_pair_tcr_embeddings)
            mlm_loss = cross_entropy_loss_fn(mlm_logits.flatten(0, 1), mlm_targets.view(-1))

            total_cont_loss += contrastive_loss.item() * num_samples
            total_mlm_loss += mlm_loss.item() * num_samples
            total_aln += metric.alignment_paired(anchor_tcr_embeddings, positive_pair_tcr_embeddings).item() * num_samples
